...
class GeometryLoss:
    ...
    def make_smoothness_constraints(self, pathObj):
        ...
        for idx, node in enumerate(self.iterate_nodes()):
            ...
            if abs(sm) < 1e-2:
                self.smooth_nodes.append(
                    (
                        node,
                        (
                            (
                                t0.norm() / self.segment_approx_length(node[0], pathObj)
                            ).item(),
                            (
                                t1.norm() / self.segment_approx_length(node[1], pathObj)
                            ).item(),
                        ),
                    )
                )
    ...
    def segment_approx_length(self, segment, pathObj):
        if segment[0] == 0:
            ...
            # should have a pair of indices now
            length = (
                pathObj.points[idxs[1], :] - pathObj.points[idxs[0], :]
            ).norm() + (pathObj.points[idxs[2], :] - pathObj.points[idxs[1], :]).norm()
            return length
        elif segment[0] == 2:
            # cubic
            idxs = self.segList[segment[0]][segment[1]]
            # should have a pair of indices now
            length = (
                (pathObj.points[idxs[1], :] - pathObj.points[idxs[0], :]).norm()
    ...
    def get_segments(self, pathObj):
        ...
        for ncp in pathObj.num_control_points.numpy():
            if ncp == 0:
                ...
                self.cubics.append(
                    (idx, (idx + 1), (idx + 2), (idx + 3) % total_points)
                )
                idx += ncp + 1
    ...
    def make_parallel_constraints(self, pathObj):
        ...
        self.parallel_groups = [
            sgroup[1]
            for sgroup in slopes
            if len(sgroup[1]) > 1
            and (
                not self.xyalign
                or (sgroup[0] > 1e-3 and abs(sgroup[0] - (math.pi / 2)) > 1e-3)
            )
        ]
    ...
    def calc_smoothness_loss(self, loss, pathObj):
        for node, tlengths in self.smooth_nodes:
            ...
            tl = (
                (t0.norm() / self.segment_approx_length(node[0], pathObj)) - tlengths[0]
            ).pow(2) + (
                (t1.norm() / self.segment_approx_length(node[1], pathObj)) - tlengths[1]
            ).pow(2)
            loss += tl * 10
    ...
    def compute(self, pathObj):
        if pathObj.id != self.pathId:
            raise ValueError(
                "Path ID {} does not match construction-time ID {}".format(
                    pathObj.id, self.pathId
                )
            )
